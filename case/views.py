...
from .serializers import (
    ...
    MortgageNeedsSerializers, CompletedCaseStatsSerializer,
)
...
class CompletedMortgageCaseStatsView(ListAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = CompletedCaseStatsSerializer
    ...
    def get_queryset(self):
        ...
        filters = {
            ...
            "case_stage": CaseStageChoices.COMPLETION,
            "is_removed": False,
        }
    ...
    def get(self, request, *args, **kwargs):
        ...
        current_count = queryset.filter(
            completed_date__gte=self.current_start,
            completed_date__lte=self.now
        ).count()

        previous_count = queryset.filter(
            completed_date__gte=self.previous_start,
            completed_date__lt=self.current_start
        ).count()
        ...
        serializer = self.get_serializer(data={
            "label": "Completed Mortgage Cases",
            "count": current_count,
